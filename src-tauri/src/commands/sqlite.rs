                ...
                let value: Result<serde_json::Value, _> = match row.get_ref(i) {
                    Ok(value_ref) => {
                        match value_ref {
                            ...
                            rusqlite::types::ValueRef::Integer(i) => Ok(serde_json::Value::Number(serde_json::Number::from(i))),
                            ...
                            rusqlite::types::ValueRef::Text(s) => {
                                match std::str::from_utf8(s) {
                                    ...
                                    Err(_) => Ok(serde_json::Value::String(format!("<binary data: {} bytes>", s.len()))),
                                }
                            },
                            rusqlite::types::ValueRef::Blob(b) => {
                                Ok(serde_json::Value::String(format!("<binary data: {} bytes>", b.len())))
                            },
                        }
                    },
                    Err(e) => Err(e),
