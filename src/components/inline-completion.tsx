...
const InlineCompletion = ({
  ...
  useEffect(() => {
    ...
    const getCursorPosition = () => {
      ...
      const paddingLeft =
        parseInt(window.getComputedStyle(textarea).paddingLeft) || 8;
      const paddingTop =
  ...
  useEffect(() => {
    if (visible && cleanCompletion) {
      console.log("🎯 Showing completion:", {
        visible,
        cleanCompletion:
          cleanCompletion.substring(0, 50)
          + (cleanCompletion.length > 50 ? "..." : ""),
        cursorPosition,
  ...
  return (
    <div
      ...
      title="Press Tab to accept, Esc to dismiss"
    >
      {cleanCompletion}
    </div>
  );
