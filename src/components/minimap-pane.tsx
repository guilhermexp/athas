...
export default function MinimapPane({
  ...
  const updateViewportFromTextarea = useCallback(() => {
    ...
    const scrollRatio =
      textareaScrollTop / (textareaScrollHeight - textareaHeight);
    ...
    const clampedTop = Math.max(
      0,
      Math.min(newViewportTop, availableScrollSpace),
    );
  ...
  const handleMinimapClick = useCallback(
    (e: React.MouseEvent) => {
      if (!textareaRef.current || !minimapRef.current || isViewportDragging)
        return;
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isViewportDragging && textareaRef.current && minimapRef.current) {
        ...
        const minimapContentHeight =
          minimapRef.current.clientHeight - headerHeight;
        const maxViewportTop = Math.max(
          0,
          minimapContentHeight - viewportHeight,
        );

        const newViewportTop = Math.max(
          0,
          Math.min(dragStartViewportTop + deltaY, maxViewportTop),
        );
