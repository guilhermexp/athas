...
export const useVim = ({
  ...
  const [lastFindDirection, setLastFindDirection] = useState<
    "forward" | "backward"
  >("forward");
      ...
      if (vimMode === "normal") {
        ...
        if (pendingOperator) {
          ...
          switch (e.key) {
            ...
            case "e": // operator + e
              motionEnd =
                findEndOfWord(activeBuffer.content, selectionStart + 1) + 1;
              break;
            case "b": // operator + b
            ...
            case "$": // operator + $
              ...
              motionEnd =
                lineEndPos === -1 ? activeBuffer.content.length : lineEndPos;
              break;
            case "0": // operator + 0
              motionEnd =
                activeBuffer.content
            ...
            case pendingOperator:
              ...
              const operatorLineEndPos =
                lineEnd === -1 ? activeBuffer.content.length : lineEnd + 1;
        ...
        switch (e.key) {
          ...
          case "A":
            ...
            const lineEnd = activeBuffer.content.indexOf("\n", selectionStart);
            const endPos =
              lineEnd === -1 ? activeBuffer.content.length : lineEnd;
          ...
          case "V":
            ...
            const vLineEndPos =
              vLineEnd === -1 ? activeBuffer.content.length : vLineEnd;
          ...
          case "w":
            ...
            const nextWordPos = findNextWordStart(
              activeBuffer.content,
              selectionStart + 1,
            );
          ...
          case "W":
            ...
            const nextBigWordMatch = activeBuffer.content
              .slice(selectionStart)
              .match(/\s+\S/);
            if (nextBigWordMatch) {
              const newPos =
                selectionStart
                + nextBigWordMatch.index!
                + nextBigWordMatch[0].length
                - 1;
              textarea.setSelectionRange(newPos, newPos);
              updateCursorPosition();
            }
          ...
          case "e":
            // Move to end of word
            const endWordPos = findEndOfWord(
              activeBuffer.content,
              selectionStart + 1,
            );
          ...
          case "E":
            ...
            const endBigWordMatch = activeBuffer.content
              .slice(selectionStart)
              .match(/\S+/);
            if (endBigWordMatch) {
              const newPos =
                selectionStart
                + endBigWordMatch.index!
                + endBigWordMatch[0].length
                - 1;
              textarea.setSelectionRange(newPos, newPos);
              updateCursorPosition();
            }
          ...
          case "b":
            ...
            const prevWordPos = findPrevWord(
              activeBuffer.content,
              selectionStart,
            );
          ...
          case "^":
            ...
            const lineStartCaret =
              activeBuffer.content
                .substring(0, selectionStart)
                .lastIndexOf("\n") + 1;
            const currentLineCaret = activeBuffer.content
              .slice(lineStartCaret)
              .split("\n")[0];
          ...
          case "D":
            ...
            const endPosD =
              lineEndPosD === -1 ? activeBuffer.content.length : lineEndPosD;
            const newContentD =
              activeBuffer.content.slice(0, selectionStart)
              + activeBuffer.content.slice(endPosD);
          ...
          case "C":
            ...
            const endPosC =
              lineEndPosC === -1 ? activeBuffer.content.length : lineEndPosC;
            const newContentC =
              activeBuffer.content.slice(0, selectionStart)
              + activeBuffer.content.slice(endPosC);
          ...
          case ",":
            if (lastFindChar) {
              const oppositeDirection =
                lastFindDirection === "forward" ? "backward" : "forward";
              const newPos = findCharInLine(
                activeBuffer.content,
                selectionStart,
                lastFindChar,
        ...
        switch (e.key) {
          ...
          case "y":
            // Copy selection
            const copiedText = activeBuffer.content.slice(
              selectionStart,
              selectionEnd,
            );
          ...
          case "w":
            const nextWordPosV = findNextWordStart(
              activeBuffer.content,
              selectionEnd,
            );
          ...
          case "e":
            const endWordPosV =
              findEndOfWord(activeBuffer.content, selectionEnd) + 1;
            textarea.setSelectionRange(selectionStart, endWordPosV);
            break;
          case "b":
            const prevWordPosV = findPrevWord(
              activeBuffer.content,
              selectionEnd,
            );
          ...
          case "$":
            ...
            const endPosV =
              lineEndPosV === -1 ? activeBuffer.content.length : lineEndPosV;
