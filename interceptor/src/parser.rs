...
impl ParsedResponse {
   ...
   fn append_partial_json_to_last_block(&mut self, partial_json: &str) {
      if let Some(last_block) = self.content.last_mut()
         && last_block.content_type == "tool_use"
      {
         ...
         match &mut last_block.input {
            ...
            Some(_) => {
               // this is being fired A LOT after every tool use and i don't know why (commenting
               // it out for now) log::warn!("Unexpected: tool_use block
               // already has parsed input");
            }
            None => {
               last_block.input = Some(serde_json::Value::String(partial_json.to_string()));
            }
         }
      }
   }
